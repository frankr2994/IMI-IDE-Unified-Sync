...
// Gives IMI + Gemini CLI the power to control Chrome directly
// ═══════════════════════════════════════════════════════════
let puppeteerProcess = null;
let puppeteerCallbacks = new Map();
let puppeteerMsgId = 1;
let puppeteerBuffer = '';

function startPuppeteerServer() {
  if (puppeteerProcess && !puppeteerProcess.killed) return Promise.resolve(true);
  return new Promise(async (resolve, reject) => {
    const npxPath = await checkCommand('npx');
    ...
    puppeteerBuffer = '';
    puppeteerProcess.stdout.on('data', (chunk) => {
      puppeteerBuffer += chunk.toString();
      const lines = puppeteerBuffer.split('\n');
      puppeteerBuffer = lines.pop();
      for (const line of lines) {
        if (!line.trim()) continue;
        try {
          const msg = JSON.parse(line.trim());
          const cb = puppeteerCallbacks.get(msg.id);
          if (cb) { puppeteerCallbacks.delete(msg.id); cb(null, msg); }
        } catch (e) { /* ignore non-JSON output during startup */ }
      }
    });
    puppeteerProcess.stderr.on('data', (d) => console.log('[Puppeteer MCP]', d.toString().trim()));
    puppeteerProcess.on('error', (e) => reject(e));
    // Give the MCP server ~3s to spin up and download Chromium if needed
    setTimeout(() => {
      // Send MCP initialize handshake
      const initMsg = JSON.stringify({ jsonrpc: '2.0', id: 0, method: 'initialize', params: { protocolVersion: '2024-11-05', capabilities: {}, clientInfo: { name: 'IMI', version: '1.0.4' } } });
      puppeteerProcess.stdin.write(initMsg + '\n');
      resolve(true);
    }, 3000);
  });
}

function callPuppeteerTool(toolName, toolArgs) {
  return new Promise((resolve, reject) => {
    if (!puppeteerProcess || puppeteerProcess.killed) return reject(new Error('Puppeteer server not running'));
    const id = puppeteerMsgId++;
    puppeteerCallbacks.set(id, (err, msg) => {
      if (err) return reject(err);
      if (msg.error) return reject(new Error(msg.error.message || JSON.stringify(msg.error)));
      resolve(msg.result);
    });
    const msg = JSON.stringify({ jsonrpc: '2.0', id, method: 'tools/call', params: { name: toolName, arguments: toolArgs } });
    puppeteerProcess.stdin.write(msg + '\n');
    // Timeout after 30s
    setTimeout(() => {
      if (puppeteerCallbacks.has(id)) {
        puppeteerCallbacks.delete(id);
        reject(new Error('Puppeteer tool call timed out'));
      }
    }, 30000);
  });
}

ipcMain.handle('puppeteer:launch', async () => {
  try {
    await startPuppeteerServer();
    return { success: true, message: '🌐 Puppeteer browser ready! Gemini CLI can now control Chrome.' };
  } catch (e) {
    return { success: false, error: e.message };
  }
});

ipcMain.handle('puppeteer:action', async (event, { action, params }) => {
  try {
    await startPuppeteerServer();
    let result;
    switch (action) {
      case 'navigate':
        result = await callPuppeteerTool('puppeteer_navigate', { url: params.url });
        return { success: true, data: result };
      case 'screenshot':
        result = await callPuppeteerTool('puppeteer_screenshot', { name: params.name || 'imi-screenshot', selector: params.selector });
        return { success: true, data: result };
      case 'click':
        result = await callPuppeteerTool('puppeteer_click', { selector: params.selector });
        return { success: true, data: result };
      case 'fill':
        result = await callPuppeteerTool('puppeteer_fill', { selector: params.selector, value: params.value });
        return { success: true, data: result };
      case 'evaluate':
        result = await callPuppeteerTool('puppeteer_evaluate', { script: params.script });
        return { success: true, data: result };
      case 'hover':
        result = await callPuppeteerTool('puppeteer_hover', { selector: params.selector });
        return { success: true, data: result };
      default:
...
ipcMain.handle('puppeteer:stop', () => {
  if (puppeteerProcess && !puppeteerProcess.killed) {
